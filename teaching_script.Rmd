...
A short recap of what we have done so far. Going back to the Data Analysis workflow I showed you at the beginning, we imported data using the `readr` package, we tidy and transform our data to make it "clean" and now we are going to visualize data using the `ggplot2` package, also part of the `tidyverse`. 
...
str(ca) ## overall structure of your dataframe
...
Finally, to know about the content of each column the function `unique()` will print all the unique values in a row.
```{r}
unique(ca$park_name) ## all national parks in CA
unique(ca$type) ## all data comes from national parks
unique(ca$region) ## PW, pacific west
```
...
`ggplot2` is a plotting package that makes it simple to create complex plots from data in a data frame. It provides a more programmatic interface for specifying what variables to plot, how they are displayed, and general visual properties. Therefore, we only need minimal changes if the underlying data change or if we decide to change from a bar plot to a scatterplot. 
...
As I mentioned earlier, to build a ggplot, we need to:
...
- provide a geom (how do we want ggplot2 to plot our data). Graphical representation of the data in the plot (points,
      lines, bars)
